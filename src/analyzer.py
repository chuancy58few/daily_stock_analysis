...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if openai_key_valid:
            ...
            elif not self._anthropic_client and gemini_key_valid:
                try:
                    self._init_model()
                except Exception as e:
        ...
        if self._use_openai:
            try:
                ...
                if self._anthropic_client:
                    logger.warning(
                        "[OpenAI] All retries failed, falling back to Anthropic"
                    )
                    return self._call_anthropic_api(prompt, generation_config)
                raise openai_error
            ...
            snapshot.update(
                {
                    ...
                    "pe_ratio": self._format_price(realtime.get("pe_ratio")),
                    "dividend_yield": self._format_percent(
                        realtime.get("dividend_yield")
                    ),
